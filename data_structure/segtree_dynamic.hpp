...
namespace RangeQuery {
    ...
    struct MinDynamic {
        using Type = struct SegTreeDynamic<Monoid::Min<T, max_value>>;
    };

    /// @brief 1点変更 / 区間 max
    template <typename T, T min_value = -INF>
    struct MaxDynamic {
        using Type = struct SegTreeDynamic<Monoid::Max<T, min_value>>;
    };

    /// @brief 1点変更 / 区間和
    template <typename T>
    struct SumDynamic {
        using Type = struct SegTreeDynamic<Monoid::Sum<T>>;
    };
}  // namespace RangeQuery
