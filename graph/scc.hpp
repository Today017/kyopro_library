...
SccInfo<T> SccDecomposition(const Graph<T>& G) {
    ...
    rep(s,N) if(!seen[s]) {
        ...
        while(!stk.empty()) {
            ...
            if(it[u]<(ii)adj.size()) {
                ii v=adj[it[u]++].to;
                if(!seen[v]) { seen[v]=true; stk.push_back(v); }
            } else {
                order.push_back(u);
                stk.pop_back();
            }
    ...
    for(ii s: order) if(comp[s]==-1) {
        ...
        while(!stk.empty()) {
            ...
            if(it[u]<deg) {
                ...
                if(comp[v]==-1) { comp[v]=idx; stk.push_back(v); }
            } else {
                stk.pop_back();
            }
        }
    ...
    sort(all(edges)); edges.erase(unique(all(edges)), edges.end());
    Graph<T> ret2(nN);
