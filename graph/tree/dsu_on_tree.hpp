...
struct DsuOnTree {
    DsuOnTree(const vector<vi>& g, int root) : n(g.size()), G(g), heavy(n, -1), root(root) {
        build();
    }
    ...
    void run(auto&& Add, auto&& Query, auto&& Reset) {
        auto efs = [&](auto&& efs, int now, int pre) -> void {
            for(int nxt : G[now])
                if(nxt != pre) efs(efs, nxt, now);
            Add(now);
        };

        auto dfs = [&](auto&& dfs, int now, int pre) -> void {
            for(int nxt : G[now])
                if(nxt != pre && nxt != heavy[now]) {
                    dfs(dfs, nxt, now);
                    Reset();
                }
            ...
            for(int nxt : G[now])
                if(nxt != pre && nxt != heavy[now]) efs(efs, nxt, now);
