...
struct Hash {
    ...
    void init() {
        if(flag) return;
        flag = true;
        base = vector<ll>(HASH_PRIME);
        rep(i, HASH_PRIME) base[i] = Xor128(3000, mod[i]);
        inv = vector<vector<ll>>(HASH_PRIME);
        pow = vector<vector<ll>>(HASH_PRIME);
        rep(i, HASH_PRIME) {
            pow[i] = vector<ll>(HASH_MAX + 1);
            inv[i] = vector<ll>(HASH_MAX + 1);
            pow[i][0] = 1;
            inv[i][HASH_MAX] = ModInv(ModPow<ll>(base[i], HASH_MAX, mod[i]), mod[i]);
            rep(j, HASH_MAX) {
